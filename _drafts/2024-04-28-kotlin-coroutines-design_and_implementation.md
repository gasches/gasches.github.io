...
CPS-трансформации делают реализацию `intrinsic`-функции `suspendCoroutineUninterceptedOrReturn` очень простой: она передает синтетический аргумент continuation в полученный блок кода и возвращает `COROUTINE_SUSPENDED`, когда это необходимо.
