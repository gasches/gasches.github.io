...
Однако, начиная с 2000-х, всё больше языков программирования начали поддерживать асинхронность.
...
_Future-и_ \[[10](#ref10)\] или _promise-ы_ \[[24](#ref24)\] (также иногда называемые _deferred-ами_ или _task-ами_) стоят на ступень выше вычислений на основе callback-ов, работая как специальные прокси для еще не завершённых результатов асинхронных вычислений[^1].
...
Расширением концепции promise-ов является их _организация в виде конвейера_ (pipelining): этот термин был введен в обиход благодаря языку программирования Joule \[[54](#ref54)\], но сама идея изначально сформулирована в \[[33](#ref33)\].
...
В нём используются наименования из стандартной библиотеки Java 8 \[[56](#ref56)\] (а именно из класса `CompletableFuture`, который реализует конвейерный promise);
...
Первым широко используемым языком программирования, в котором появились _async/await_, был C# версии 5.0 \[[22](#ref22)\] (2012 год)[^2].
...
В C# имеется два дополнительных ограничения касающихся реализации _async/await_.
...
Единственное, что намекает на его асинхронную природу, - это ключевые слова _async/await_.
...
Хотя мы еще вернёмся к этому решению в дизайне и попытаемся объяснить его в [разделе 4.2](#42-дизайн), важно отметить, что, несмотря на эти различия, реализации C++ и Kotlin по-прежнему относятся к подходу _async/await_.
...
Однако представление функций, поддерживающих асинхронное выполнение, уже существует и это - корутины \[[19](#ref19), [37](#ref37)\] (coroutines, сопрограммы).
...
Проект OpenJDK Loom \[[1](#ref1), [7](#ref7)\] стремится достичь эту амбициозную цель в JVM (Java Virtual Machine).
...
_Стековая_ реализация корутин поддерживает приостановку внутри _произвольно_ вложенных функций;
...
_Бесстековая_ реализация, с другой стороны, может приостанавливать только текущую корутину;
...
Это позволяет компилятору минимизировать объем генерируемого кода по сравнению с функциональным вариантом CPS \[[18](#ref18)\] в котором на каждый вызов обычно генерируются по несколько continuation-объектов.
