...
Cимптомы сложной системы:
* лавинообразные изменения - простые доработки влекут многочисленные изменения в кодовой базе системы;
* высокая когнитивная нагрузка - внесение изменений в систему требует изучения чрезмерно большого количества информации;
...
В любом случае результатом будет _неопределеность_.
...
Утечка информации возникает когда проектное решение проявляется в нескольких модулях, что также увеличивает степень зависимости между ними. Примером являются отдельные модули для записи и чтения файла в определенном формате.
...
* В каких ситуациях используется данный метод? Используется ли он лишь единожды?
...
1) _транзитные методы_ - методы, делегирующие вызов другому методу со схожей или идентичной сигнатурой практически не добавляя, при этом, функциональности системе в целом (пример: _неглубокие_ декораторы, анти-пример: диспетчеры и _глубокие_ декораторы);
2) внутренние представления модуля не отличаются от абстракций предоставляемых интерфейсом;
3) _транзитные переменные_ - переменные передающиеся вниз по цепочке методов (техники для устранения: перенос в общий объект, глобальные переменные, создание объекта-_контекста_).
...
* реализация смежной функциональности в разных компонентах создает _дистанцию_, которая затрудняет понимание системы и внутренних зависимостей;
...
Создать исключительную ситуацию легко, а корректно её обработать - сложно. Поэтому лучшим способом уменьшения сложности, вызванной обработкой ошибок, является минимизация таких мест. Для этого могут быть использовны следующие техники:
...
* аггрегация - обработка нескольких типов исключительных ситуаций в одном месте;
...
Проектирование ПО - сложная задача. Рассмотрение нескольких альтернативных вариантов для каждого важного решения позволит достичь намного более лучших результатов.
...
Всегда будет что-то кажующееся более приоритетным, чем написание документации и комментариев.
...
* при документировании переменных сосредоточтесь на том, что они представляют, а не как используются.
...
* сокращение числа итераций при работы над кодом реализации.
...
Но, при таком подходе, код постепенно становится все сложней. **Если вы не сделали дизайн системы лучше, то, скорее всего, вы сделали его хуже.**
...
Правила поддержания акутальности комментариев:
...
Одна из главных концепций _agile_ заключается в том, что разработка должа быть инкрементной и итеративной.
...
Если шаблон проектирования хорошо работает в конкретной ситуации, то, скорее всего, другой подход не приведёт к более лучшему результату.
...
Для обеспечение удовлетворительной производительности системы необходимо с самого начала понимать какие операции, по своей природе, являются _дорогими_. Примеры:
